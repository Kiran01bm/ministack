...
### Fixed
- **RDS — Aurora engine versions are validated on non-create writes and global inheritance** — `ModifyDBInstance`, `ModifyDBCluster`, `CreateGlobalCluster`, and `ModifyGlobalCluster` stored arbitrary Aurora engine versions that the shared engine catalog did not advertise, and `CreateDBCluster` could inherit such a version from legacy global-cluster state even though supplying it explicitly was rejected. All paths now use the shared catalog validator and reject unknown versions before mutating any state: create paths keep the create-time `Cannot find version {version} for {engine}`, while the modify paths report the upgrade-target shape real AWS uses (`InvalidParameterCombination` / `Cannot find upgrade target from {current} with requested version {requested}.`, as seen in real `ModifyDBCluster` transcripts). `ModifyGlobalCluster` now also propagates an accepted version to every member cluster the way real AWS applies global-database version changes, and `ModifyDBCluster` refuses to move a member cluster to a different major than its global cluster — the combination `CreateDBCluster` already refuses to produce. Contributed by @kiran01bm.
### Added
- **RDS — replicated Aurora PostgreSQL readers survive StopDBCluster / StartDBCluster and warm boot** — stopping a cluster with replicating readers was refused with `InvalidDBClusterStateFault` (the follow-up slice of #1325 the reader feature shipped without), and a MiniStack restart demoted a persisted reader to an alias of the writer's shared container. `StopDBCluster` now stops the writer's container and every reader-owned standby container — a stop that fails on any of them rolls back what it stopped and reports the failure instead of publishing `stopped` — and `StartDBCluster` restarts the writer, waits for it to accept authenticated connections, then revives each reader by re-cloning from the writer at its current address, keeping the member's identity, endpoint, and volume name (reader compute owns no durable state, as on Aurora, so revival recreates the container rather than restarting it). The same revival runs on warm boot. A revival that genuinely fails lands the cluster back on `stopped` for retry; only when compute deliberately cannot come back (control-plane-only restore, or the replication flag withdrawn between runs) does the member fall back to aliasing the writer, releasing its former container and volume, instead of advertising an endpoint nothing serves. Contributed by @kiran01bm.
### Fixed
- **RDS — global Aurora stop/start preserves topology and MySQL replication** — `StopDBCluster` and `StartDBCluster` allowed individual members of a multi-cluster Aurora global database to transition independently, and deleting a primary's last instance stopped shared compute that secondaries still needed. Starts and stops are now limited to sole-member global databases (`InvalidDBClusterStateFault`, 400), last-instance deletion preserves compute while other global members exist, and a successful recreate resets stale MySQL replica state before reconfiguring or deferring replication after readiness without overwriting a deliberate legacy-volume failure. Contributed by @kiran01bm.
### Added
