...
### Added
- **RDS — `FailoverDBCluster`** — forcing an Aurora cluster failover was `InvalidAction`. The operation now promotes a reader member to writer: an explicit `TargetDBInstanceIdentifier` must be an available reader belonging to the cluster (a non-member is `InvalidDBInstanceState`, an unknown instance `DBInstanceNotFound`); without a target the available reader with the lowest `PromotionTier` is chosen, as on AWS. The response reports the transitional `failing-over` status while a follow-up `DescribeDBClusters` sees `available` with the flipped `IsClusterWriter` flags, mirroring `FailoverGlobalCluster`. A missing cluster is `DBClusterNotFoundFault`, a non-available cluster or one with no promotable reader `InvalidDBClusterStateFault`. Metadata-only for now: cluster members share one container, so there is no reader data plane to re-point until per-instance replication (#1325) lands. Contributed by @kiran01bm.
- **RDS — opt-in Aurora PostgreSQL reader instances backed by real streaming replication** — with `MINISTACK_RDS_PG_CLUSTER_REPLICATION=1`, the second and later members of an Aurora PostgreSQL cluster launch their own PostgreSQL containers, cloned from the writer with `pg_basebackup` and running as hot standbys that stream WAL from it. A reader is genuinely read-only (`pg_is_in_recovery()` is true, writes fail with `25006`), sees data written to the writer, and the cluster `ReaderEndpoint` resolves to an available reader instead of the writer — so read/write connection routing, replica-lag handling, and reader-endpoint pooling can be rehearsed locally. Off by default: without the flag (and for Aurora MySQL) members keep aliasing the writer's single shared container. Requires the shared Docker network (`DOCKER_NETWORK`); deleting a reader removes only its own container. Not yet modeled (follow-up slices of #1325): stopping a cluster with replicating readers is rejected with `InvalidDBClusterStateFault`, and a persisted reader demotes to a shared-container alias on warm boot. Contributed by @kiran01bm.
...
### Fixed
- **CloudFormation — `AWS::IoT::Policy` updates apply instead of rolling the stack back** — the type had a create and a delete but no update handler, so a stack update fell through to create, hit `ResourceAlreadyExistsException` on the policy it had provisioned itself, and rolled back with the document unchanged: an IoT policy could be deployed once and never edited again. A changed `PolicyDocument` is now a no-interruption update stored as a new policy version and made the default, so `Ref` keeps naming the same policy and `GetPolicy` returns the new document; a changed `PolicyName` is a replacement, creating the policy under the new name and removing the old one. Because IoT caps a policy at five versions, the update prunes the oldest non-default versions before storing a new document, as the CloudFormation handler does on AWS.
...
- **S3 — versioning edge cases: the null version, delete-marker signals, and versioned copies** — five behaviors around the edges of object versioning now match AWS. A PUT on a Suspended bucket minted a fresh UUID version id like an Enabled one; it is now stored as the `null` version — replacing any previous null version, answered without `x-amz-version-id` — and a suspended DELETE mints its marker under the literal `null` id the same way. The object written before versioning was enabled used to vanish from the index at the first versioned overwrite; it now stays addressable as `VersionId=null` and becomes current again when the newer version is deleted. Delete markers announce themselves: `DeleteObjects` without a per-key VersionId on a versioned bucket now creates a marker (for keys that never existed too) and reports `DeleteMarker`/`DeleteMarkerVersionId`, and a GET/HEAD 404 for a key hidden by a marker carries `x-amz-delete-marker: true` with the marker's id. `UploadPartCopy` ignored the copy source's `?versionId=` and copied the current bytes; it now copies that exact version and echoes `x-amz-copy-source-version-id`. A versioned `CopyObject` dropped the source version's user metadata (the index kept it; the synthesized source object just didn't carry it). And both copy operations decoded the copy-source header before splitting at `?`, so a key legitimately named `?versionId` — sent percent-encoded — read as an empty key with a version qualifier; the raw header is split first now. The five version-minting sites (PutObject, POST, CopyObject, CompleteMultipartUpload, and the delete-marker path) shared five hand-copied bookkeeping blocks; they now go through one helper, which is what makes the null-version rules uniform. Contributed by @gaul.
- **S3 — `CompleteMultipartUpload` honors `If-Match` / `If-None-Match`** — conditional writes landed on PutObject, but CompleteMultipartUpload accepted the same headers and ignored them, so a create-once (`If-None-Match: *`) or compare-and-swap (`If-Match: <etag>`) upload that took the multipart path silently overwrote whatever was there. The complete now evaluates the same preconditions PutObject does, against the current object at complete time — 412 on a violated condition, 404 `NoSuchKey` when If-Match addresses a key with no current object. That last rule also tightens PutObject: the `*` form previously answered the RFC 7232 412 for a missing key, but AWS documents a single If-Match error row — `NoSuchKey` — and ceph/s3-tests pins 404 for both forms, so both operations now answer it. The idempotent complete-retry replay still bypasses the check, since the original request passed its conditions when it committed. Contributed by @gaul.
- **S3 — canned ACLs are stored, and object ACLs bind to versions** — SDKs send `put-bucket-acl --acl public-read` as an `x-amz-acl` header with an empty body, and `PutBucketAcl` read only the body, so the canned grant vanished and the bucket read back owner-only — a caller that gates anonymous access on the reported grants (S3Proxy's `aws-s3` backend among them) then treated every bucket as private. `CreateBucket`'s `x-amz-acl` was dropped the same way. Both now validate the canned value (`InvalidArgument` otherwise) and store the grants it implies, `log-delivery-write` included, exactly as the object side has since #1322; an XML body must be well-formed (`MalformedACLError`) and a request with neither header nor body answers `MissingSecurityHeader`, as on AWS. Object ACLs are now per-version like tags (#1058): `?versionId=` reads or writes that version's ACL, a fresh version starts from the default policy instead of inheriting its predecessor's, the canned value on `PutObject` sticks to the version that PUT created, and version and bucket deletes drop the stored entries. The browser-POST form names its field `acl` rather than `x-amz-acl`, so it never matched the header list and was discarded; it now applies to the uploaded object. Contributed by @gaul.
